  ...
  <div class="calculator-border">
    ...
    <div class="calculator-items">
      ...
      <div class="calculator-row-3">
        ...
        <div class="calculator-item" @click="operate('*')">*</div>
        <div class="calculator-item" @click="devide">1/x</div>
      </div>
...
export default {
  ...
  methods: {
    ...
    // slice方法： slice(0, -1) 表示从开始到倒数第一个字符（不包括最后一个）
    drop() {
      console.log("drop..");
    ...
    square() {
      ...
      eval(this.formula) < 0
        ? (this.formula = "Can not squre ther negative value")
...
.calculator .calculator-border .calculator-items .calculator-item {
  ...
  background-color: rgba(224, 135, 135, 0.78);
